...
class AuthInterceptor extends Interceptor{
  ...
  onRequest(RequestOptions options) {
    ...
    LogFsUtil.d("interctpter:"+accessToken);
...
class TokenInterceptor extends Interceptor{
  ...
  Future<String> getToken() async {
    ...
    params["refresh_token"] = SpUtil.getString(BaseConstant.refresh_Token);
    try{
...
class LoggingInterceptor extends Interceptor{
  ...
  onRequest(RequestOptions options) {
    ...
    if (options.queryParameters.isEmpty){
      LogFsUtil.i("RequestUrl: " + options.baseUrl + options.path);
    }else{
      LogFsUtil.i("RequestUrl: " + options.baseUrl + options.path + "?" + Transformer.urlEncodeMap(options.queryParameters));
    }
    LogFsUtil.d("RequestMethod: " + options.method);
    LogFsUtil.d("RequestHeaders:" + options.headers.toString());
    LogFsUtil.d("RequestContentType: ${options.contentType}");
    LogFsUtil.d("RequestData: ${options.data.toString()}");
    return super.onRequest(options);
  }
  ...
  onResponse(Response response) {
    ...
    if (response.statusCode == ExceptionHandle.success){
      LogFsUtil.d("ResponseCode: ${response.statusCode}");
    }else {
      LogFsUtil.e("ResponseCode: ${response.statusCode}");
    }
    // 输出结果
    LogFsUtil.json(response.data.toString());
    LogFsUtil.d("----------End: $duration 毫秒----------");
...
class AdapterInterceptor extends Interceptor{
  ...
  static const String SUCCESS_FORMAT = "{\"resultCode\":200,\"data\":%s,\"resultMessage\":\"\"}";
